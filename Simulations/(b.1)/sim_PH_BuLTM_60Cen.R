...
load('PH_data_high.RData')
load('PH_data_high_test.RData')

beta0 <- c(1, 1, 1)

beta0 <- beta0 / norm(beta0, '2')#real beta 
grids <- seq(.1, 2.6, by = .01)
...
for(j in 1:100){
  ...
  for(i in 1:n_test){
    real_curve[i, ] <- 1 - pexp(as.numeric(exp(-(beta0) %*% t(X_test[i, ]))) * as.numeric(sapply(grids, f)))
  }
  
  
  A <- BuLTM(Y, X, TMdataT$delta, probseries = seq(0, 1, .25), K = 12, iter = 1500, a = 1, eta = .01, zeta = .5)
  ...
  write.table(outres, file = 'Apr192025_PHHigh_BuLTM.txt',
