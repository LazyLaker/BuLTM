...
load('BoxNonlinMix.RData')
load('BoxNonlinMix_test.RData')
...
for(j in 1:100){
  ...
  S1 <- bSpline(X$x1, knots = quantile(X$x1, seq(0.1, .9, .2)), Boundary.knots = c(-2, 2))
  
  S2 <- bSpline(X$x2, knots = quantile(X$x2, seq(0.1, .9, .2)), Boundary.knots = c(-2, 2))
  
  TX <- cbind(S1, S2)
  
  delta <- TMdataT$delta
  
  model <- BuLTM(TY, TX, delta = delta, probseries = seq(0, 1, .2), 
                 K = 12, a = 1, eta = .01, zeta = .5, 
                 iter = 1200, warmup = 400, 
                 max_treedepth = 9, adapt_delta = .7)
  ...
  for(i in 1:n_test){
    ...
    real_curve[i, ] <- 1- p(distr)(-zi+ as.numeric(sapply(grid, sign_bc)))
  }
  ...
  write.table(outres, file = 'June2025_NonlinMix.txt',
