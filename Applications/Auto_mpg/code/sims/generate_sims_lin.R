...
run_sims <- function(seed, its, burnin,  pnon, m, a = 1, b = 0.001){
  ...
  res <- bctm(      
    ...
    iterations = 2000, warmup = 1000, burnin = 1000,
    hyperparams=list(a=1, b=.001), nuts_settings=list(adapt_delta = 0.9, max_treedepth=16), seed = 1234)
  
  
  exp_ident <-res$model$exp_ident
  
  # beta samples
  betas <- res$samples$beta[(1000+1):2000, ]
