...
for(i in 1:m){
  ...
  ctmm<- ctm(response = Bernstein_basis(MPG_var, order = 4, ui = 'increasing'), shifting = ~cylinders + displacement + 
  ...
  write.table(outres, 'MPG_mlt_Lin.txt', col.names = T, row.names = F)
